...
class RenderObjectsView {
  ...
  #initObjectComparisonBar() {
    ...
    this.#elements.comparisonAnchor.addEventListener('click', function () {
      ...
      // Guard 2
      if (n > maxItems) {
        this.children[0].innerHTML = 'Bitte weniger Objekte auswählen';
        setTimeout(function () {
          button.children[0].innerHTML = _this.#initialComparisonAnchorText;
        }, 1200);
        return;
  ...
  #objectComparisonCheckBoxClickHandler(data: any, checkBox: HTMLInputElement) {
    ...
    setTimeout(() => {
      ...
      } else if (!itemExists && _this.#objectComparisonItems.length >= 5) {
        _this.#objectComparisonCheckBoxClickHandlerPause = true;
        checkBox.click();
        setTimeout(function () {
          _this.#objectComparisonCheckBoxClickHandlerPause = false;
  ...
  #manipulateClone(el = document.body, data: any) {
    ...
    const yearString =
      data.data.endofconstruction !== null
        ? data.data.endofconstruction
        : data.data.rentbegin;
    const constructionYearString =
      yearString !== null ? yearString.split('-')[0] : 'k.A.';
    ...
    if (units)
      units.innerHTML = data.stats.numUnits ? data.stats.numUnits : 'k.A.';
    ...
    if (yieldFromTo) yieldFromTo.innerHTML = yieldFromToText;
  ...
  #loadMoreClickHandler(
    ...
    gsap.to(window, {
      ...
      scrollTo: {
        y: '[filters="wrapper"]',
        offsetY: this.#elements.nav.offsetHeight + 16,
      },
    });
  }
    ...
    elements.objectLists.forEach(
      (
        ...
        if (updatedObjects.length < 1) {
          if (isPaginationCall && index + 1 === numberOfLists) {
